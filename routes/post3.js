...
var addpost3 = function(req, res) {
	...
	if (database.db) {
		...
		database.UserModel.findByEmail(paramWriter, function(err, results) {
			...
			// save()로 저장
			// PostModel3 인스턴스 생성
			var post = new database.PostModel3({
				title: paramTitle,
			...
			post.savePost(function(err, result) {
			    ...
			    return res.redirect('/process/showpost3/' + post._id); 
			});
...
var listpost3 = function(req, res) {
	...
	if (database.db) {
		...
		database.PostModel3.list(options, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				// 전체 문서 객체 수 확인
				database.PostModel3.count().exec(function(err, count) {
					...
					req.app.render('listpost3', context, function(err, html) {
                        if (err) {
                            console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
...
var showpost3 = function(req, res) {
	...
	if (database.db) {
		// 1. 글 리스트
		database.PostModel3.load(paramId, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				req.app.render('showpost3', context, function(err, html) {
					if (err) {
                        console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
...
module.exports.listpost3 = listpost3;
module.exports.addpost3 = addpost3;
module.exports.showpost3 = showpost3;
