...
var addpost2 = function(req, res) {
	...
	if (database.db) {
		...
		database.UserModel.findByEmail(paramWriter, function(err, results) {
			...
			// save()로 저장
			// PostModel2 인스턴스 생성
			var post = new database.PostModel2({
				title: paramTitle,
			...
			post.savePost(function(err, result) {
			    ...
			    return res.redirect('/process/showpost2/' + post._id); 
			});
...
var listpost2 = function(req, res) {
	...
	if (database.db) {
		...
		database.PostModel2.list(options, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				// 전체 문서 객체 수 확인
				database.PostModel2.count().exec(function(err, count) {
...
var showpost2 = function(req, res) {
	...
	if (database.db) {
		// 1. 글 리스트
		database.PostModel2.load(paramId, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				req.app.render('showpost2', context, function(err, html) {
					if (err) {
                        console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
...
module.exports.showpost2 = showpost2;
