...
var addpost4 = function(req, res) {
	...
	if (database.db) {
		...
		database.UserModel.findByEmail(paramWriter, function(err, results) {
			...
			// save()로 저장
			// PostModel4 인스턴스 생성
			var post = new database.PostModel4({
				title: paramTitle,
			...
			post.savePost(function(err, result) {
			    ...
			    return res.redirect('/process/showpost4/' + post._id); 
			});
...
var listpost4 = function(req, res) {
	...
	if (database.db) {
		...
		database.PostModel4.list(options, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				// 전체 문서 객체 수 확인
				database.PostModel4.count().exec(function(err, count) {
					...
					req.app.render('listpost4', context, function(err, html) {
                        if (err) {
                            console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
...
var showpost4 = function(req, res) {
	...
	if (database.db) {
		// 1. 글 리스트
		database.PostModel4.load(paramId, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				req.app.render('showpost4', context, function(err, html) {
					if (err) {
                        console.error('응답 웹문서 생성 중 에러 발생 : ' + err.stack);
...
module.exports.listpost4 = listpost4;
module.exports.addpost4 = addpost4;
module.exports.showpost4 = showpost4;
